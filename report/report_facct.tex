...
or ``unknowable,'' depending on choices no reader can see, and every wrong
verdict we measured erred toward the clean, reassuring direction. An audit
...
flags those definitions turn on. That is what makes lending the right place to
measure a gap that is usually unmeasurable: the gap between the construct a
fairness instrument is \emph{named for} and the construct it actually
\emph{operationalises} \citep{jacobswallach2021}.

This paper's claim is that the gap appears at every layer of an audit, that it
takes the same shape at each layer, and that it is invisible in the output.
...
\subsection{Layer one: what the library operationalises}
\label{sec:library}
...
\subsection{Layer two: what the documented construct defines}
\label{sec:statute}
...
\subsection{Measurement validity: what the number can support}
\label{sec:validity}
...
\subsection{Layer three: what the audit's own instruments operationalise}
...
\section{Discussion: why the gap is invisible}
\label{sec:invisible}
...
\section{Recommendations: what an audit must record}
\label{sec:protocol}
